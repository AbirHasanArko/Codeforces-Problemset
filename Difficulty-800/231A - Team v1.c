...
Input:
The first input line contains a single integer n (1 ≤ n ≤ 1000) — the number of problems in the contest. Then n 
lines contain three integers each, each integer is either 0 or 1. If the first number in the line equals 1, then 
Petya is sure about the problem's solution, otherwise he isn't sure. The second number shows Vasya's view on the 
...
know how to solve the second problem. That means that they will write solutions for these problems. Only Petya 
...
int main()
{
    ...
    for(int i = 1; i <= n; i++){
        ...
        if(petya + vasya + tonya >= 2){    // If at least two of the inputs are 1, sum of the three inputs >= 2
            count += 1;
        }
    }
    printf("%d", count);
...
// 221310543	Aug/31/2023 21:18UTC+6	RoronoaArko	231A - Team	GNU C11	Accepted	30 ms	0 KB
