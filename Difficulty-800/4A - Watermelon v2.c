...
#include<stdio.h>
int main(){
    int w, k = 0;   // if k = 0 is not initiated, k may automatically store the value 1, which will create problem
    scanf("%d", &w);
    for(int i = 1; i <= w; i++){
        for(int j = 1; j <= w; j++){
            if(i % 2 == 0 && j % 2 == 0 && i + j == w){     // Checks if i and j are even and if their sum == w
                k = 1;
                break;    // terminates the nested for loop if finds a match
            }
        }
    }
    if(k == 1){
...
//  221168647	Aug/30/2023 23:00UTC+6	RoronoaArko	4A - Watermelon	GNU C11	Accepted	30 ms	0 KB
